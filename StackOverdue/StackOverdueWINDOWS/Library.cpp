...
//        cout <<"insert to mmapOfCountAuthor: count="<<count<<"; author="<<str_next<<"\n";
    ...
    multimap<int, string>::reverse_iterator ritr1, ritr2;
...
//        cout <<"insert to mmapOfCountGenre: count="<<count<<"; genre="<<str_next<<"\n";
    ...
    if (ritr1 != mmapOfCountGenre.rend()) {
        ...
        ret = mmapOfCountGenre.equal_range(ritr1->first);
        mmapOfCountGenre.erase(ret.first,ret.second);

        ritr2=mmapOfCountGenre.rbegin();
        if (ritr2 != mmapOfCountGenre.rend()) {
            mostReadGenre2=ritr2->second;
        }
    }
